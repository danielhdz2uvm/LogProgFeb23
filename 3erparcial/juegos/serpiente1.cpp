...
class Fruta{
        public:
                int x,y;
                char cuerpo=254; // Caracter de fruta
};
...
void Juego::genSnake(){
        ...
        snake[0].dx=1;
    snake[0].dy=0;
        snake[0].cuerpo=157; //Cabeza de serpiente
    ...
    for(i=1;i < tam; i++){
        ...
        snake[i].cuerpo=184; //Cuerpo de la serpiente
    }
...
void Juego::cfruta(){
        if(snake[0].x==fruta.x && snake[0].y==fruta.y){
                genFruta();
        tam+=1;
        snake[tam-1].cuerpo=184; // Cambiar tipo de cuerpo
        score+=1;
        if(tam%10==0){
                        nivel++;
                        velocidad-=5; //Aumento de velocidad
                }
...
void Juego::main(){
        system("color 0a"); //Cambiar el color de elementos
        gameover=false;
        portada();
        system("cls");
        score=1;
        velocidad=110; //Velocidad inicial
        tam=4; //Tamaño inicial
